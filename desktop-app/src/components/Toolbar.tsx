...
import { mediaTypeForField } from '@/config/workflowConfig';
...
function categoryOf(workflow: ServerWorkflow): Category {
  const text = `${workflow.id} ${workflow.kind || ''}`.toLowerCase();
  if (workflow.kind === 'audio' || /音频|声音|audio|fish/.test(text)) return '音频';
  if (workflow.kind === 'digital_human' || /数字人|对口型|infinitetalk|动作迁移/.test(text)) return '数字人';
  if (workflow.kind === 'video' || /视频|ltx|wan2|首尾帧|animate|短剧/.test(text)) return '视频';
  if (workflow.kind === 'image' || /文生图|生图|图像|qwen|krea|zimage|高清|放大|分镜|洗图|多视图|三视图/.test(text)) return '图像';
  return '其他';
}
...
function workflowFields(remote: Awaited<ReturnType<typeof fetchWorkflowConfig>>) {
  ...
  return Object.keys(enabled).filter(key => enabled[key]).map(key => {
    ...
    const node = template[nodeId] || {}; const value = values[key];
    ...
    return { key, label: labels[key] || field, value, field, nodeTitle: node._meta?.title || `节点 ${nodeId}`, fileType, type: typeof value };
  });
}
...
export const Toolbar: React.FC = () => {
  ...
  if (!open) return <Tooltip title="打开节点库"><Button className="node-library-toggle" icon={<AppstoreOutlined/>} onClick={() => setOpen(true)}>节点库</Button></Tooltip>;
  return <aside className="node-library">
    ...
    <div className="node-library__quick">
