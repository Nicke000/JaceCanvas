...
const SYSTEM_PROMPTS: Record<PromptKind, string> = {
...
export async function optimizePrompt(input: { text: string; action: PromptAction; kind: PromptKind; imageContext?: string[] }): Promise<string> {
  ...
  const task = input.action === 'translate' ? '请将下面提示词翻译成自然、适合生成模型使用的英文，只输出英文提示词。' : SYSTEM_PROMPTS[input.kind];
  ...
  const body = s.optimizerProvider === 'gemini'
    ? { contents: [{ role: 'user', parts: [{ text: `${SYSTEM_PROMPTS[input.kind]}\n${user}` }] }] }
    : s.optimizerProvider === 'anthropic'
      ? { model: s.optimizerModel, max_tokens: 1200, system: SYSTEM_PROMPTS[input.kind], messages: [{ role: 'user', content: user }] }
      ...
      ? { model: s.optimizerModel, stream: false, prompt: user, system: SYSTEM_PROMPTS[input.kind], images: imageUrls.filter(url => /^data:image\//i.test(url)).map(url => url.split(',')[1]), options: { num_ctx: 4096 } }
        : { model: s.optimizerModel, temperature: 0.35, metadata: { ai_canvas_request_id: requestId, new_conversation: true }, messages: [{ role: 'system', content: SYSTEM_PROMPTS[input.kind] }, { role: 'user', content: imageUrls.length ? openAiContent : user }] };
